...
f = udf(lambda x: x[:4], ArrayType(StringType()))
df = df.withColumn('c_i', f('c_i')) \
       .selectExpr('acct_no', 'member_id', 'c_i rec_list')

# 热门列表
item_hot_list = ins_in_sale[:4]
...
# 存表
try:
    df.write.saveAsTable('fbidm.tdm_firs_rec_20171220_4_ins_pc', mode='overwrite')
except:
    print('----------\n %s \n----------' % 'Fail!!!')
else:
    print('----------\n %s \n----------' % 'Success!!!')
